...
<div class="between mb">
    <div>
        ...
        <p class="muted">Pilih soal yang akan ditambahkan ke ujian: <b>{{ $exam->title }}</b>. Soal yang ditambahkan akan menjadi salinan ujian, sehingga aman walaupun bank soal diedit lagi nanti.</p>
    </div>
    ...
    <form class="table-toolbar" method="GET" action="{{ route('exams.question-bank.select', $exam) }}">
        ...
        <div class="table-tools">
            <div class="tool-field search">
                ...
                <div class="live-search-wrap"><input class="input" data-live-search="selectBankTable" name="q" value="{{ request('q') }}" placeholder="Cari soal, mapel, topik"></div>
            </div>
...
<form method="POST" action="{{ route('exams.question-bank.add', $exam) }}" class="card data-card">
    ...
    <div class="table-wrap"><table class="table" id="selectBankTable"><thead><tr><th></th><th>Soal</th><th>Jenis</th><th>Opsi/Kunci</th><th>Mapel/Topik</th><th>Akses/Pembuat</th><th>Poin</th></tr></thead><tbody>
        @forelse($items as $item)
            <tr>
                <td><input type="checkbox" name="question_bank_ids[]" value="{{ $item->id }}"></td>
        ...
        @empty<tr><td colspan="7">Belum ada soal di bank.</td></tr>@endforelse
    </tbody></table></div>
